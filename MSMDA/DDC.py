...
class DDC():
    def __init__(self, model=models.DDC(), source_loader=0, target_loader=0, batch_size=64, iteration=10000, lr=0.001, momentum=0.9, log_interval=10):
        self.model = model
...
def cross_subject(data, label, session_id, subject_id, category_number, batch_size, iteration, lr, momentum, log_interval):
    ...
    target_loader = torch.utils.data.DataLoader(dataset=utils.CustomDataset(target_data, target_label),
                                                ...
                                                shuffle=True,
                                                drop_last=True)
    model = DDC(model=models.DDC(pretrained=False, number_of_category=category_number),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DDC parameters')
    parser.add_argument('--dataset', type=str, default='seed3',
                        help='the dataset used for DDC, "seed3" or "seed4"')
    ...
    # data preparation
    print('Model name: DDC. Dataset name: ', dataset_name)
