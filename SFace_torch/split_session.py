...
def train_test_split_per_subject_cross_trial(
    ...
    if not os.path.exists(split_path):
        ...
        trial_ids = subject_info['trial_id'].unique()
        ...
        # train_trial_ids = np.where(np.isin(trial_ids,train_index_trial_ids))[0]
        # test_trial_ids = np.where(np.isin(trial_ids,test_index_trial_ids))[0]


        train_info = []
        # for train_trial_id in train_trial_ids:
        for train_trial_id in train_index_trial_ids:
            train_info.append(
        ...
        for test_trial_id in test_index_trial_ids:
            test_info.append(
